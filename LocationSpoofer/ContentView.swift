...
struct SettingsView: View {
    @EnvironmentObject var kernelManager: KernelLocationManager
    private var ddiManager: DDIMountManager { DDIMountManager.shared }
    ...
    var body: some View {
        NavigationView {
            List {
                ...
                Section {
                    ddiStatusRow
                    if case let .failed(err) = ddiManager.ddiStatus {
                        Text(err)
                            .font(.caption)
                            .foregroundColor(.red)
                            .lineLimit(5)
                    ...
                    Text("On iOS 17+ the Developer Disk Image is personalized per-device and can only be mounted by Xcode (not from a sideloaded app). Location spoofing still works without it via Darksword kernel R/W (VFS plist + heap-patch + RemoteCall). The StikDebug com.apple.dt.simulatelocation path will activate automatically if you've already mounted the DDI via Xcode.")
                        .font(.caption)
                }
    ...
    private var ddiStatusColor: Color {
        if ddiManager.statusIsOk { return .green }
        if case .personalizationRequired = ddiManager.ddiStatus { return .orange }
        if case .failed = ddiManager.ddiStatus { return .red }
        return .secondary
    }
    ...
    private var ddiStatusRow: some View {
        HStack(spacing: 14) {
            ZStack {
                Circle()
                    .fill(ddiStatusColor.opacity(0.15))
                    .frame(width: 42, height: 42)
                Image(systemName: ddiManager.statusIcon)
                    .font(.system(size: 18, weight: .semibold))
                    .foregroundColor(ddiStatusColor)
            }
    ...
    private var ddiTitle: String {
        if ddiManager.statusIsOk { return "Mounted" }
        if case .personalizationRequired = ddiManager.ddiStatus { return "Not Required" }
        if case .failed = ddiManager.ddiStatus { return "Check Failed" }
        return "Not Mounted"
    }
